...
With any large open source project, dozens of pull requests and issues are always in flight, which can make it hard to see the bigger picture. Starting this quarter, we will publish regular roadmap updates to give you a clear view of what the [conda maintainers](https://github.com/orgs/conda/teams/conda-maintainers) team from Anaconda and Quansight, together with the broader community, is building and what you can expect.  
...
### More Consistency: Input File Handling Refactor  

**What we heard:** Conda supports different input files like `environment.yml` and `requirements.txt`, but behavior can feel inconsistent. Switching between formats or mixing them can cause confusion and errors.  

**What we are working on:** We are completing a refactor to unify **input file handling** under a plugin-based system. This will make conda’s behavior more consistent and create a cleaner foundation for supporting new formats in the future.  

**How you will benefit:** You will see more predictable behavior no matter which file type you start from, reducing surprises when switching between formats. For contributors, this refactor will make it easier to extend conda with new input formats, allowing the ecosystem to evolve more quickly.  
...
**What we heard:** Installing or updating packages can be slow because conda downloads a single large index of all available packages, even if you only need a small part. This leads to delays in local workflows, CI pipelines, and on slower networks.  

**What we are working on:** We are implementing support for **sharded repodata**, as proposed and accepted in [Conda Enhancement Proposal (CEP) 16](https://conda.org/learn/ceps/cep-0016). Instead of fetching one massive file, conda will be able to download smaller, targeted shards of metadata.  
...
- **[Contribute to conda](https://github.com/conda/conda/blob/main/CONTRIBUTING.md)** or even become a maintainer yourself!
