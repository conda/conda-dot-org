---
title: "Sharded repodata in conda (beta): an order of magnitude faster fetches"
slug: "sharded-repodata-improvements"
authors: [travishathaway]
tags: [conda, performance]
description: "Conda fetches repodata much more quickly. Learn how we did it and who helped along the way."
image: img/blog/2025-12-02-sharded-repodata-improvements/banner.jpg
---
...
Repodata is a conda specific term that refers to the package index that all
conda clients must
download in order to find and install available packages. The best way to think about it is as
a database containing every package file, its dependencies, and other metadata for a particular channel.
...
<!-- packages+packages.conda incl. noarch: main/linux-64 126708; conda-forge/linux-64: 978983 -->
...
as they could be. This led the engineers at [prefix-dev](https://prefix.dev) who work on the
[pixi package manager](https://pixi.sh) to innovate and draft [CEP 16](https://conda.org/learn/ceps/cep-0016),
with the help of the entire conda community, that defines a new mechanism for fetching repodata using a sharded
approach.

This approach works by splitting repodata into multiple "shards". Each package has its
...
### Bringing sharded repodata to conda and conda-forge

CEP 16 defined a clear way to speed up solves, and we’ve been doing the work needed to make that a reality in conda.
...
- [anaconda.org](https://anaconda.org) team at Anaconda worked with the conda-forge community to enable hosting the new repodata files. We plan to work with other channels to add support as the beta progresses.
...
To compare performance between the sharded and non-sharded approach, we used two different
environment creation scenarios: **Python** and **Data Science**. The Python scenario fetches the package
`python` and all its dependencies, and the Data Science scenario fetches the packages, `pandas`, `plotly` and `scipy` and their dependencies. We benchmarked
...
:::info
We used a temporary **conda-forge-sharded** channel because at the time of
...
Furthermore, we ran our comparison using a **cold** cache where nothing was present in conda's
...
To get a complete picture of how these changes affect conda, we not only measured **total time**
...
- Non-sharded fetching always has to fetch the same sized repodata but
  sharded repodata only fetches what it needs, meaning it varies based on the requested packages as seen
  here.
...
Both package scenarios see significant decreases in memory usage with a fifteen and
and seventeen times reduction in memory usage.
