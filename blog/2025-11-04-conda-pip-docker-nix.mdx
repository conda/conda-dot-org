...
- **ELF surgery via [`patchelf`](https://github.com/NixOS/patchelf)** *(the tool originated in the Nix ecosystem and is widely used by `conda-build` and `rattler-build`)*
...
| **Unit of publication** | Wheel/sdist (library) | Distro package (build + run metadata) | Layered image | Derivation output (hashed closure) |
