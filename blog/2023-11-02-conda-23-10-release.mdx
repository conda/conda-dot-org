---
title: "Conda 2023.10: libmamba is now the default solver"
...
libmamba builds on [libsolv](https://github.com/openSUSE/libsolv), a much faster solver from openSUSE. The previous "classic" solver is based on [pycosat](https://github.com/conda/pycosat)/[Picosat](http://fmv.jku.at/picosat/) and will remain part of conda for the foreseeable future. A [fallback is possible and available](/blog/2023-11-02-conda-23-10-release#what-can-i-do-if-this-update-doesnt-work-for-me).
...
* Third, the text output from running `conda` is changing slightly. In addition to some error messages being different, running conda may now produce a superset of text compared to what was previously produced. If you parse conda's output text then you may need to update your scripts.
