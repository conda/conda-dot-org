...
relatively small in size this works pretty well. But, for larger channels like conda-forge, this
has meant longer download times for users as this channel has continued to grow over the years.
...
### What did this mean for the `conda` package manager?
...
profiling, sharded repodata was not available on conda-forge on anaconda.org.

The benchmarks that were used to create the following plots were run inside a `linux/arm64`
Docker container running on an Apple M1 Pro.
:::
...
Both package scenarios see significant decreases in memory usage with 15 times and 17 times reductions in memory usage.
