...
## Changes in Conda [24.7.0](https://github.com/conda/conda/releases/tag/24.7.0)

To update conda, run:

```bash
conda install -n base conda=24.7.0
```
...
- Print transaction report for `@EXPLICIT` lockfile installs too.
...
- Disallow some more characters in Windows for prefix names (`^`, `%`, `!`, `=`, `(`, `)`, `\`). These characters complicate or prevent environment activation if present.
...
- Add cheatsheet PDF download to cheatsheet page.
