...
These projects prove the ideas are sound, and many of them can and should keep evolving. Several are incubating in [`conda-incubator`](https://github.com/conda-incubator) today. When a piece is ready, `conda` can take it on as a dependency and ship it as the default behavior without necessarily merging the whole implementation into the main `conda` codebase. My vision is that we keep investing in both the plugin ecosystem and core `conda`.
