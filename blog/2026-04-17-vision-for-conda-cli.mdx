...
authors: [danyeaw]
tags: [conda, vision]
description: "A vision for the conda CLI in 2026: making conda fast, trusted, and delightful."
image: img/blog/2026-04-17-vision-for-conda-cli/banner.png
...
<ThemedImage
  alt="conda CLI 1.4 in action"
  sources={{
    light: useBaseUrl("/img/blog/2026-04-17-vision-for-conda-cli/conda-1-4-light.gif"),
    dark: useBaseUrl("/img/blog/2026-04-17-vision-for-conda-cli/conda-1-4-dark.gif"),
  }}
/>
<br />

Fast-forward five years to 2017: `conda` 4.4 shipped with both Python 3.6 and 2.7 installers and included a major cleanup of the CLI. One large change was new `conda activate` and `conda deactivate` commands, which replaced `source activate` / `source deactivate` (on Linux/macOS) and `activate` / `deactivate` (on Windows). It also added a new spinner and used the [`tqdm`](https://github.com/tqdm/tqdm) library for progress bars. Those pieces are still in `conda` today.

<ThemedImage
  alt="conda CLI 4.4 in action"
  sources={{
    light: useBaseUrl("/img/blog/2026-04-17-vision-for-conda-cli/conda-4-4-light.gif"),
    dark: useBaseUrl("/img/blog/2026-04-17-vision-for-conda-cli/conda-4-4-dark.gif"),
  }}
/>
<br />
...
In 2024, `conda` gained a reporter backend plugin, and Travis Hathaway put together a demo of what a more modern CLI could look like with the [conda-rich](https://github.com/conda-incubator/conda-rich) incubator project.

<ThemedImage
  alt="conda CLI with conda-rich in action"
  sources={{
    light: useBaseUrl("/img/blog/2026-04-17-vision-for-conda-cli/conda-rich-light.gif"),
    dark: useBaseUrl("/img/blog/2026-04-17-vision-for-conda-cli/conda-rich-dark.gif"),
  }}
/>
<br />

In January 2025, Jaime Rodríguez-Guerra started [conda-spawn](https://github.com/conda-incubator/conda-spawn), in the spirit of `poetry shell` and `pixi shell`: spawn a subshell with the environment already active instead of activating in place, so you can skip `conda init` hooks on every login. That keeps ordinary shell startup light and defers loading `conda` until you are ready to use it.

Finally, in December 2025, `conda` shipped [sharded repodata](https://conda.org/blog/sharded-repodata-improvements) in beta, a fundamental step forward for `conda` by cutting how much metadata you download and process.
...
Speed is about perception as much as raw performance. [Nielsen's research](https://www.nngroup.com/articles/response-times-3-important-limits/) on response times shows that feedback within 100 ms feels instant and anything over a second creates uncertainty. In other words, silence is the enemy of responsiveness.
...
- Rattler integration – We would like to use [Rattler](https://github.com/conda/rattler) more, a high-performance Rust implementation of core conda primitives. The performance floor rises significantly when the resolver and file I/O are written in Rust.

Our goal isn't to win a benchmark, but to make conda feel fast and responsive. Jaime Rodríguez-Guerra created a demo called [`conda-ng`](https://github.com/jaimergp/conda-ng/) that demonstrates what a blazingly fast `rattler`-based `conda` might look like:

<ThemedImage
  alt="Conda CLI in action"
  sources={{
    light: useBaseUrl("/img/blog/2026-04-17-vision-for-conda-cli/conda-ng-light.gif"),
    dark: useBaseUrl("/img/blog/2026-04-17-vision-for-conda-cli/conda-ng-dark.gif"),
  }}
/>
<br />
...
Here is an example based on Jannis Leidel's proof-of-concept [`conda-express`](https://github.com/jezdez/conda-express) of what a future Miniconda installer could look like as a lightweight, single-binary:

<ThemedImage
  alt="Conda CLI in action"
  sources={{
    light: useBaseUrl("/img/blog/2026-04-17-vision-for-conda-cli/miniconda-light.gif"),
    dark: useBaseUrl("/img/blog/2026-04-17-vision-for-conda-cli/miniconda-dark.gif"),
  }}
/>
<br />
...
[`conda-lockfiles`](https://github.com/conda-incubator/conda-lockfiles) is launching in the next few weeks in beta. It adds support for multiplatform lockfile formats, including `conda-lock` and `pixi.lock`, making cross-platform environment reproducibility a first-class `conda` feature.

[`conda-self`](https://github.com/conda-incubator/conda-self) is also launching in beta soon. It adds a `conda self` command that lets you safely manage your `base` environment, including protecting it from accidental modifications.

The following three projects are proof-of-concepts from Jannis Leidel that explore what future `conda` workflows could look like:

[`conda-workspaces`](https://github.com/conda-incubator/conda-workspaces) brings a pixi-like project-based workflow to `conda`, letting you manage multi-environment workspaces and tasks from a single configuration file.

[`conda-global`](https://github.com/conda-incubator/conda-global) enables installing CLI tools into isolated environments and placing them on your PATH, similar to `pipx install`, `uv tool`, or `pixi global`.

[`conda-express`](https://github.com/jezdez/conda-express) is the lightweight, single-binary bootstrapper shown above. My vision is that it becomes the future Miniconda and Miniforge installer themselves, a small static binary that gets you a fully functional `conda` installation in seconds.

These projects prove the ideas are sound. But plugins can only go so far, and the next step is bringing these same improvements into `conda` itself.
