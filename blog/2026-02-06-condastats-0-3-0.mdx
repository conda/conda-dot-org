---
title: "condastats is back: query conda download stats from your browser"
slug: "condastats-0-3-0"
authors: [jezdez]
...
The headline feature of this release cycle is an **interactive browser demo** that lets you query conda download statistics without installing anything. It runs [Pyodide](https://pyodide.org/) (CPython compiled to WebAssembly) directly in your browser, with the real `condastats` package installed via micropip.

The demo includes:

- A **Python REPL** with syntax highlighting, tab completion, and command history
- **15 curated example queries** -- click a card to explore "Scientific Python," "AI/ML frameworks," "Top 20 packages," and more
- **Chart.js visualizations** that render download trends as interactive charts
- **Shareable URLs** -- copy a link to any query and share it with colleagues
- **Quick date range presets** (1 month, 3 months, 6 months, 1 year, 2 years)

The demo is served directly from the [condastats documentation](https://condastats.readthedocs.io) and loads about 20 MB of Pyodide plus 1--15 MB of package data per queried month.

Give it a spin right here -- enter a package name, pick a date range, and hit **Query**:
...
*[Open the full demo in a new tab.](https://condastats.readthedocs.io/en/latest/try/)*

:::tip
The browser demo uses the new pure-pandas query API introduced in 0.4.0, which doesn't require dask or s3fs -- making it lightweight enough to run in a WebAssembly environment.
:::
...
### Polished browser demo (0.4.1, 0.4.2)

The initial Pyodide demo was refined over two quick-follow releases:

- **0.4.1** fixed an fsspec version conflict that broke `micropip install`
- **0.4.2** replaced the basic interface with a full Python REPL (jQuery Terminal + Prism.js), added 15 example query cards, shareable URLs, date range presets, and fixed a race condition in parallel Parquet loading


## Install and try it

You don't even need to install condastats to try it locally. Use `uvx` or `pipx run` to run it in a temporary environment:
...
<Tabs>
  ...
  <TabItem value="pipx" label="pipx run">
