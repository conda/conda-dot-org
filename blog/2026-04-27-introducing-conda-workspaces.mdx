...
Conda is good at managing environments and packages. What it has never been good at is the layer above them: declaring, in one canonical place, that a given codebase needs these three environments, composed from these features, locked for these platforms, and exercised by these commands. Most teams stitch that layer together by hand: an `environment.yml` per env, a `Makefile` to run things, and a separate lockfile workflow bolted on top.

[conda-workspaces 0.4.0](https://github.com/conda-incubator/conda-workspaces) ships today to fill that gap. It is a conda plugin built around the workspace: a directory whose TOML manifest (`conda.toml`, `pixi.toml`, or `pyproject.toml`) declares one or more named environments, the reusable features those environments compose from, the platforms each one targets, and the tasks that run inside them. Every declared environment becomes an ordinary conda prefix under `.conda/envs/`. Installing solves each declared platform and writes the result to a multi-platform `conda.lock`. Everything underneath stays plain conda: the same solver, package cache, and channel authentication that any other conda command uses.

<!-- truncate -->

It is also a concrete step toward the future Dan Yeaw sketched in [Building a Better conda CLI: A Vision](/blog/2026-04-17-vision-for-conda-cli): declarative project environments via `conda.toml`, a workspace-aware CLI, and reproducible cross-platform locks shipped as part of conda itself.

## “Isn’t that what pixi is for?”

It is the first thing every reviewer asks.

[pixi](https://pixi.sh) by [prefix.dev](https://prefix.dev) introduced an excellent workspace model: one TOML manifest that describes environments, features, platforms, and tasks, paired with its own Rust stack ([resolvo](https://github.com/prefix-dev/resolvo) for solving and the [rattler](https://github.com/conda/rattler) crates for conda primitives) distributed as a single binary. That manifest design is what conda-workspaces builds on. It reads the same `conda.toml` / `pixi.toml` / `pyproject.toml` and turns the environments declared in it into conda prefixes.
...
Solving every platform from a single CI job becomes the long pole as a workspace grows. The lock command is matrix-friendly: each runner emits one fragment with `--output`, and a coordinator job stitches them back together with `--merge`, with no solver runs.
...
It is not a replacement for pixi. If a self-contained, batteries-included project tool with its own solver, runtime, and cache is the right fit, [pixi](https://pixi.sh) is genuinely good and is the better answer. conda-workspaces is for teams that already live in the conda ecosystem and want pixi-style project workflows without moving everything else.
