...
- Native wheel and PyPI support – The [`conda-pypi`](https://github.com/conda/conda-pypi) plugin that improves PyPI interoperability is also about trust. The current workflow of doing `pip install` inside a conda environment undermines `conda`'s value by creating a split in dependency tracking that `conda` can't reason about. If you watch the `conda` 1.4 animation above carefully, initial versions of `conda` had a `conda pip` command. However, instead of using `pip` directly, `conda` should be able to natively understand how to install wheels, which are the Python binary format. When `conda` can install from PyPI directly and keep its dependency graph coherent, users can trust the environment they have is the environment conda describes.
