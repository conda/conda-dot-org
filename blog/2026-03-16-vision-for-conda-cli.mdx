...
After extensive discussion with stakeholders across the community, including at a maintainers offsite in Málaga, Spain, in December, and after looking at incubated and third-party projects that extend `conda` through its plugin system, I've laid out my vision for where `conda` could go. Since that offsite, other maintainers have turned several of those ideas into experiments such as `conda-ng` to help show what is possible. I invite the broader community to help build it.

**TL;DR**: The vision for `conda` in 2026 is a package manager that is blazingly fast (real-time progress, sharded repodata, Rattler integration), trusted (better error messages, smarter confirmation friction, streaming output), and delightful (sensible defaults, helpful suggestions, intentional visuals, accessibility). Beyond the core CLI, the roadmap includes native PyPI wheel support, declarative `conda.toml` project environments, single-binary installers, and a comprehensive API for IDE and agent integrations.

<!-- truncate -->

## A Brief History

Before we dive into the future, let's first spend a minute reflecting on the past. By the end of 2012, the Anaconda Distribution had about 100 packages and `conda` ran on Python 2.7. The CLI was a set of modules that used `optparse`, the precursor to `argparse`, and the progress bar library was vendored into conda. Even so, the look and feel is not all that different from modern `conda`. Below, `conda` 1.4 shows its help output and creates an environment.

<ThemedImage
  alt="conda CLI 1.4 in action"
  sources={{
    light: useBaseUrl("/img/blog/2026-03-16-vision-for-conda-cli/conda-1-4-light.gif"),
    dark: useBaseUrl("/img/blog/2026-03-16-vision-for-conda-cli/conda-1-4-dark.gif"),
  }}
/>
<br>

Fast forward five years to 2017: `conda` 4.4 shipped with both Python 3.6 and 2.7 installers and included a major cleanup of the CLI. One large change was new `conda activate` and `conda deactivate` commands, which replaced `source activate` and `source deactivate`. It also added a new spinner and used the [`tqdm`](https://github.com/tqdm/tqdm) library for progress bars. Those pieces are still in `conda` today.

<ThemedImage
  alt="conda CLI 4.4 in action"
  sources={{
    light: useBaseUrl("/img/blog/2026-03-16-vision-for-conda-cli/conda-4-4-light.gif"),
    dark: useBaseUrl("/img/blog/2026-03-16-vision-for-conda-cli/conda-4-4-dark.gif"),
  }}
/>
<br>
...
2023 was also a big year: 23.10.0 switched the default solver from the classic Python implementation to libmamba via a solver plugin, which dramatically improved how long solves took.
...
<ThemedImage
  ...
  sources={{
    light: useBaseUrl("/img/blog/2026-03-16-vision-for-conda-cli/conda-rich-light.gif"),
    dark: useBaseUrl("/img/blog/2026-03-16-vision-for-conda-cli/conda-rich-dark.gif"),
  }}
/>
<br>
...
That's the lens I would like to apply to `conda`'s vision, so let’s dive into the details as part of this post.
...
Here is an example of what a future Miniconda installer could look like as a lightweight, single-binary:

<ThemedImage
  alt="Conda CLI in action"
  sources={{
    light: useBaseUrl("/img/blog/2026-03-16-vision-for-conda-cli/miniconda-light.gif"),
    dark: useBaseUrl("/img/blog/2026-03-16-vision-for-conda-cli/miniconda-dark.gif"),
  }}
/>
